...
class SecretMenuSubstate extends MusicBeatSubstate
{
	private var curOption:SecretOption = null;
	...
	function getOptions()
	{
		...
		#if !html5
		var option:SecretOption = new SecretOption('Playback Rate', 'songspeed', 'float', 1);
		...
		var option:SecretOption = new SecretOption('Instakill on Miss', 'instakill', 'bool', false);
		optionsArray.push(option);
	}
	...
	override function update(elapsed:Float)
	{
		...
		if(nextAccept <= 0)
		{
						...
						if(pressed) {
							...
							switch(curOption.type)
							{
								...
								case 'string':
									...
									{
										var oOption:SecretOption = getOptionByName("Scroll Speed");
										if (oOption != null)
										{
			...
			if(controls.RESET)
			{
				for (i in 0...optionsArray.length)
				{
					var leOption:SecretOption = optionsArray[i];
	...
	function updateTextFrom(option:SecretOption) {
